...
func (thisUser *User) Online() {
	...
	thisUser.server.Broadcast_(thisUser, "Login")
}
...
func (thisUser *User) Offline() {
	...
	thisUser.server.Broadcast_(thisUser, "Log out")
}

func (thisUser *User) SendMessage(msg string) {
	_, err := thisUser.conn.Write([]byte(msg))
	if err != nil {
		fmt.Println("User.SendMessage write error: ", err, msg)
		return
	}
}
...
func (thisUser *User) HandleMessage(msg string) {
	if msg == "who" {
		...
		thisUser.server.Broadcast_(thisUser, msg)
	}
}
